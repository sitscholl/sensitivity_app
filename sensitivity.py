...
@st.cache
def get_analytical():
    analytical = pd.read_excel('../EU_PDO_cat_variety.xlsx', na_values = ['', 'na', 'nan'])
...
def get_sensitivity():
    tbl_sens = gpd.read_file('../results/pdo_sensitivity.gpkg')
    tbl_sens['catI'] = tbl_sens['catI'].astype(str)
    return(tbl_sens)

@st.cache
def get_shp():
    shp = gpd.read_file('../results/sensitivity_groupings.gpkg')
    return(shp)
...
st.caption('The PDO regions that fall into the same group as the selected PDO are shown below as red points. PDOs are grouped based on **Category of wine product**, **Variety** and **Climate class**. This means that all PDOs within the same group share the following characteristics: A) They produce the selected wine type using the selected variety and B) they fall into the same climate class. These PDOs are used to determine the climatic range for the selected variety.')
st.caption(f'Selected variety: **{variety_selected}**')
fig, ax = plt.subplots(subplot_kw = dict(projection = ccrs.PlateCarree()))
pdo_group.to_crs(4326).plot(ax = ax, transform = ccrs.PlateCarree(), marker = 'o', 
                            edgecolor = 'black', color = 'red', linewidth = 0.7)
ax.set_extent([25, -20, 55, 25])
...
fig, ax = plt.subplots(subplot_kw = dict(projection = ccrs.PlateCarree()))
pdo_class.to_crs(4326).plot(ax = ax, transform = ccrs.PlateCarree(), marker = 'o', 
                            edgecolor = 'black', linewidth = 0.7,
                            column = 'Sensitivity', cmap = 'viridis',
                            legend = True, legend_kwds={'shrink': 0.5},
                            vmin = 0, vmax = 1)
pdo_sel.to_crs(4326).plot(ax = ax, transform = ccrs.PlateCarree(), marker = 'o',
